...
DEFAULT_REQUIRED_ENVELOPE_FIELDS = [
    "runtime_id",
    "app_id",
    "agent_id",
    "agent_name",
...
def register_app(
    ...
    record.update({
        ...
        "required_event_envelope_fields": list(DEFAULT_REQUIRED_ENVELOPE_FIELDS),
...
def registry_rows(path: str | Path | None = None) -> List[Dict[str, Any]]:
    ...
    for app in load_registry(path).get("apps", []):
        ...
        rows.append({
            ...
            "Required Envelope Fields": len(app.get("required_event_envelope_fields", [])),
